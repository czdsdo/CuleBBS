...
import com.example.chat.service.ChannelManager;
import com.example.chat.utils.NettyUtil;
...
import static com.example.chat.protocol.CuleChatType.*;
...
public class UserAuthHandler extends SimpleChannelInboundHandler {
        ...
        if (evt instanceof IdleStateEvent){
           IdleStateEvent event = (IdleStateEvent) evt;
            ...
            if (event.state().equals(IdleState.READER_IDLE)) {
                ...
                //广播用户数量
                manager.broadMessage(com.example.chat.protocol.CuleChatProtocol.buildSysUserInfo(manager.getUsers()));
            }
        }
    }
    ...
    private void handleWebSocket(ChannelHandlerContext ctx,WebSocketFrame frame){
        ...
        com.example.chat.protocol.CuleClientProtocol clientProto = JSON.parseObject(message, new TypeReference<com.example.chat.protocol.CuleClientProtocol>(){});
        byte type = clientProto.getType();
        Channel channel = ctx.channel();
        if (clientProto.getMAGIC()!= com.example.chat.protocol.CuleChatProtocol.getMAGIC())return;//过滤Magic格式不正确的消息
        switch (type){
            case PING_CODE:
            case PONG_CODE:
            ...
            case AUTH_REQUEST_CODE:
                ...
                manager.broadMessage(com.example.chat.protocol.CuleChatProtocol.buildAuthProto(isSuccess));
                if (isSuccess)
                    manager.broadMessage(com.example.chat.protocol.CuleChatProtocol.buildSysUserInfo(manager.getUsers()));
                return;
